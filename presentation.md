...
*   **軸控**: BN-B3A-10S 可透過 EtherCAT 控制最多 10 軸馬達；整體拓撲支援最多 16 個從站。
...
*   **即時部分 (Real-time Environment)**
    *   執行於 Xenomai 即時環境。
    *   **硬體抽象層**: 支援各家 EtherCAT 從站裝置。
    *   **軸控引擎**: 包括軸組、運動學、路徑預視、補間功能。
    *   **rtForth 虛擬機**: 執行即時腳本語言，實現複雜的即時控制邏輯。

**1.14.4 生命週期與設定術語**

*   **控制器 (controller)**：目前可供 HMI 及即時操作使用的 EtherCAT 控制能力。
*   **執行世代 (runtime generation)**：控制器的一次內部執行個體；重新掃描或復原時可由新世代取代。
*   **共用草稿 (shared draft)**：正在檢查、尚未持久儲存的設定。
*   **已儲存設定檔 (saved profile)**：控制器上的持久設定；儲存不會改變作用中控制器。
*   **偵測拓撲 (detected topology)**：最新完整實體掃描的唯讀證據，不會自動成為設定。
*   **重新掃描 (rescan)**：使用上次正常運作設定重建就緒控制器。
*   **復原 (recovery)**：控制器無法使用時，檢查已儲存設定檔並啟動替代控制器。
*   **拓撲維護 (topology maintenance)**：以引導式流程檢查並核准預定的實體拓撲變更。
...
可控軸數、支援從站數及掃描容量是不同限制；16 個從站可包含 I/O、閘道及其他非軸類裝置。
...
| **執行週期**      | **2 ms**                                                     |
| **可控軸數**      | **1–10（BN-B3A-10S）**                                      |
| **支援從站數量**  | **1–16**                                                     |
| **執行期掃描容量** | **16 個 EtherCAT 從站**                                     |
| **支援驅動模式**  | PP, PV, HM, CSP, CSV                                         |

**核心功能**
* 掃描並回報 EtherCAT 從站；偵測結果不會自動成為設定。
* 支援多種廠牌的馬達驅動器與 IO 模組。
* 二軸與三軸同動及直線圓弧補間。
...
> **安全界限：**HMI 的確認、**Ready** 狀態及軟體命令都是操作或維護功能，不是安全
> 聯鎖、緊急停止或動作許可。接線、試車或執行可能產生動作的程式前，必須依現場核准
> 程序完成上鎖、動作停用、電氣隔離及驗證。
