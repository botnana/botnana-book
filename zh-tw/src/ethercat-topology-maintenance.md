...
| **Proposed topology** | 明確選擇 **Configure detected topology** 後產生的完整偵測拓撲。此處只能編輯 **Expected Alias**。 |
...
變更。檢查期間，先前的偵測資料列會標示 **Before review** 及 **Reference only**，
直到更新掃描完成後才由目前結果取代。
...
6. 讓機台進入現場核准的安全狀態。
7. 只有在可使用 **Review detected topology**，且附近沒有顯示未解決原因時才繼續。
...
1. 選擇 **Review detected topology**。
2. 閱讀確認內容；只有在機台已安全時才確認。
3. 等待 Botnana Control 釋放控制器世代及 EtherCAT master，並自動要求更新掃描。
   不要重複送出要求。
4. 更新掃描尚未完成時，確認 **Detected slaves** 保留先前資料列，並標示 **Before
   review** 及 **Reference only**。
5. 自動更新完成後，確認目前拓撲取代先前資料列。若在這個步驟重新連線，或需要
   明確重掃，仍可使用 **Refresh detected topology**。
...
3. 檢查裝置電源、EtherCAT-IN/EtherCAT-OUT 方向、纜線接合及實體順序。只有在程序
   允許時才能恢復電源。
4. 如果硬體是在自動掃描後才變更，請選擇 **Refresh detected topology** 並等待完成。
   重新整理或關閉瀏覽器不會取消控制器已接管的掃描。
5. 選擇 **Detected slaves**，確認來源指出目前的拓撲掃描。
...
如果偵測拓撲和已儲存拓撲相同，可以取消檢查並恢復先前控制器設定。如果只需要改變
預期別名，仍可建立提案。

## 建立並檢查完整提案

1. 再次確認每個偵測資料列都是預定項目。
2. 選擇 **Configure detected topology**。不能只採用部分拓撲。
...
4. 再次檢查已儲存版本及拓撲。
5. 選擇 **Start controller with saved topology**，確認訊息指出預定的已儲存變更。
6. 等待啟動。不要再送出啟動或取消要求。
...
目前狀態明確選擇 **Review detected topology**、**Start controller** 或修正設定。

## 不採用掃描結果而取消

當偵測到的硬體不應在此時啟用時，請取消檢查：

- 沒有未儲存拓撲草稿時，選擇 **Cancel and restore previous controller**。
- 檢查尚未完成時，選擇 **Cancel and discard draft**；此時不能保留草稿。
- 草稿已完整檢查時，請明確選擇 **Cancel and preserve draft** 或 **Cancel and discard
  draft**。

取消會啟動保留的先前控制器設定，不會採用偵測掃描。必要時先恢復相容的實體
硬體，再等待 **Ready** 並確認機台。

如果提案已經儲存，取消不會回復已儲存設定；已儲存拓撲會繼續保留但尚未啟用。

## 重新連線及故障處理

內部拓撲檢查工作階段屬於控制器，不屬於單一瀏覽器。重新整理或重新連線後，請等待
**Restoring topology review state** 被目前階段取代。繼續前先確認階段、狀態修訂版、草稿及
...
- **啟動或恢復失敗：**讓機台保持安全，只使用 HMI 提供的重試或先前控制器恢復操作。
