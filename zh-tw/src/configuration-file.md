...
操作人員亦可透過 HMI 的 **Download configuration** 下載目前設定檔案進行備份，或在需要時透過 **Upload configuration** 還原設定。

> **升級保護機制**：在進行 Botnana Control Debian 套件更新或升級時，系統會完整保留既有的 `/etc/botnana-control/motion.toml` 設定檔，絕不覆蓋機台已校正的參數。

HMI 尚有草稿或正在進行拓撲維護時，請勿直接於後台編輯檔案。若經授權必須手動編輯，應先依現場維修程序停止控制器，並備份完整檔案。即使設定檔語法正確，內容錯誤仍可能使控制器無法啟動或造成非預期運動設定。

儲存設定不會改變運作中的控制器。**Active controller** 會保留控制器成功啟動時選用的值，直到之後的受控啟動選用已儲存設定為止。一般 **Rescan EtherCAT** 會使用上次正常運作設定，不會使用剛儲存的值。
