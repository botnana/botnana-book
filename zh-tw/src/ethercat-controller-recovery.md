...
| 尚無復原狀態 | 等待 HMI 建立連線並收到 `bnc-motion` 的最新狀態；不要送出另一項要求。 |
| HMI 要求重新啟動 `bnc-motion` | 依照[清理失敗時](#清理失敗時)處理。 |

**尚無復原狀態** 表示 HMI 尚未從目前連線的 `bnc-motion` 程序收到最新、可作為
操作依據的復原能力報告。這通常會在剛開啟或重新整理頁面、WebSocket 重新連線，
或 `bnc-motion` 重新啟動時短暫出現。這不代表已儲存設定遺失，也不一定表示控制器
已失敗。在收到報告以前，HMI 會讓復原操作保持不可用，避免依照快取或推測的狀態
執行動作。如果連線恢復後仍持續顯示此狀態，請重新整理頁面，並確認 `bnc-motion`
正在執行。
