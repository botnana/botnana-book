...
- `motion/Cargo.toml` currently uses product version `1.14.4` and package
  revision `19`; additional 1.14.4 package revisions may be required before
  final release closeout.
- `debian/changelog` finalizes revisions `1.14.4-2` through `1.14.4-19` for
  ...
  adds bounded structured topology traces for field diagnosis.
...
- [x] Confirm that all commits through `5bdeba96b` on
...
- [x] Finalize the control repository Debian changelog through `1.14.4-19`.
